...
<nav class="navbar navbar-expand-lg navbar-dark custom-navbar fixed-top">
    <div class="container-fluid px-4">
        ...
        <!-- ==== LOGO LINK BASED ON USER ROLE ==== -->
        <a class="navbar-brand d-flex align-items-center" href="<?php
            $role = session()->get('role');
            if ($role === 'admin') {
                echo base_url('/admin'); // === IF ADMIN, REDIRECT TO ADMIN DASHBOARD ===
            } elseif ($role === 'customer') {
                echo base_url('/customer'); // === IF CUSTOMER, REDIRECT TO CUSTOMER HOMEPAGE ===
            } else {
                echo base_url('/');
            }
        ?>">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            ...
            <ul class="navbar-nav mx-auto">
                ...
                <!-- ==== CUSTOMER-SPECIFIC NAVBAR LINKS ==== -->
                <?php if (session()->get('role') === 'customer'): ?>
                    <li class="nav-item">
                        <a class="nav-link" href="<?= base_url('customer/shop') ?>">Shop</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="<?= base_url('customer/product') ?>">Products</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="<?= base_url('customer/about') ?>">SMISKI</a>
                    </li>
                <?php endif; ?>
                
                <!-- ==== LINK FOR ALL LOGGED-IN USERS (ADMIN AND CUSTOMER) ==== -->
                <li class="nav-item">
